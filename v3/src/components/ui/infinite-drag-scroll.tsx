    ...
    <GridVariantContext.Provider value={variant}>
      <div className="h-dvh overflow-hidden relative" id="memoirs">
        ...
        <div className="absolute top-12 left-0 right-0 z-10 pointer-events-none text-center">
          ...
          <p className="text-sm md:text-lg text-zinc-400 max-w-xl mx-auto leading-relaxed shadow-black drop-shadow-lg">
            A massive visual log. Click, hold, and drag the grid around!
          </p>
        </div>
...
export const GridItem = ({
  ...
  const gridItemStyles = cva(
    "overflow-hidden hover:cursor-pointer w-full h-full will-change-transform relative group",
    {
      variants: {
        variant: {
          default: "rounded-sm",
          masonry: "even:mt-[60%] rounded-xl shadow-xl",
          polaroid:
  ...
  return (
    <motion.div
      ...
      animate="animate"
    >
      <div className="absolute inset-0 bg-transparent group-hover:bg-cyan-500/20 z-10 transition-colors duration-300 rounded-xl" />
      {children}
    </motion.div>
  );
};

...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent | TouchEvent) => {
      ...
      cardStreamState.current.velocity = Math.max(Math.abs(delta) * 5, cardStreamState.current.minVelocity);
      if (delta !== 0) {
        cardStreamState.current.direction = Math.sign(delta) as 1 | -1;
      }
    };
    const handleMouseUp = () => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY !== 0) {
        cardStreamState.current.direction = Math.sign(-e.deltaY) as 1 | -1;
      }
    };
  ...
  }, [isPaused, cards, cardGap, friction, scanEffect, direction, initialSpeed]);
    ...
    <div className="relative w-full overflow-hidden" style={{ height: '400px' }}>
      ...
      <div className="absolute w-full h-full flex items-center justify-center">
        <div ref={cardLineRef} className="flex items-center whitespace-nowrap cursor-grab select-none will-change-transform" style={{ gap: `${cardGap}px` }}>
          {cards.map(card => (
            <div key={card.id} className="card-wrapper relative w-[350px] md:w-[400px] h-[220px] md:h-[250px] shrink-0">
